...
class PanelAnalysisResult:
    ...
    # (Optional) ?멸렇硫섑뀒?댁뀡 留덉뒪??    mask: Optional[np.ndarray] = None
    ...
    def area_percentage(self) -> float:
        """?대?吏 ?鍮??⑤꼸 ?곸뿭 鍮꾩쑉 (異뷀썑 怨꾩궛)"""
        return 0.0


class SolarPanelPipeline:
    """
    ?쒖뼇愿??⑤꼸 Dual Track AI 遺꾩꽍 ?뚯씠?꾨씪??    
    Modes:
    ...
    def __init__(
        ...
        print(f"DEBUG: Dual Track AI ?뚯씠?꾨씪??珥덇린??以?.. (Threshold: {yolo_conf_threshold})")
        
        # 1. CCTV Track Components
        print(f"DEBUG: CCTV Track 紐⑤뜽 濡쒕뱶 以?from: {cctv_yolo_path}")
        ...
        try:
            print(f"DEBUG: Enhancer 紐⑤뜽 濡쒕뱶 ?쒕룄: {esrgan_model_path}")
            self.enhancer = RealESRGANEnhancer(model_path=esrgan_model_path)
            print("DEBUG: Enhancer 紐⑤뜽 濡쒕뱶 ?깃났")
            
            print(f"DEBUG: SegFormer 紐⑤뜽 濡쒕뱶 ?쒕룄: {segformer_model_path}")
            self.seg_classifier = SegFormerClassifier(model_path=segformer_model_path)
            print("DEBUG: SegFormer 紐⑤뜽 濡쒕뱶 ?깃났")
        except Exception as e:
            print(f"ERROR: CCTV Track 紐⑤뜽 濡쒕뱶 ?ㅽ뙣 (Enhancer/SegFormer): {e}")
            logger.error(f"CCTV Track 紐⑤뜽 濡쒕뱶 ?ㅽ뙣 (Enhancer/SegFormer): {e}")
            import traceback
            traceback.print_exc()
            self.enhancer = None
        ...
        print(f"DEBUG: Drone Track 紐⑤뜽 濡쒕뱶 以?from: {drone_yolo_path}")
    ...
    def _analyze_drone(self, image: np.ndarray) -> List[PanelAnalysisResult]:
        """?쒕줎 ?뚯씠?꾨씪?? YOLO(High) -> Keras"""
        logger.info("?? Drone Pipeline ?쒖옉")
        results = []
    ...
    def _analyze_cctv(self, image: np.ndarray) -> List[PanelAnalysisResult]:
        """CCTV ?뚯씠?꾨씪?? YOLO(Low) -> Enhancer -> SegFormer"""
        logger.info("?렏 CCTV Pipeline ?쒖옉")
        results = []
        
        if not self.enhancer or not self.seg_classifier:
            logger.error("CCTV 紐⑤뜽??濡쒕뱶?섏? ?딆븘 遺꾩꽍???섑뻾?????놁뒿?덈떎.")
            return []
...
def get_pipeline() -> SolarPanelPipeline:
    ...
    if _pipeline_instance is None:
        # 紐⑤뜽 寃쎈줈 ?ㅼ젙
        # app/ai/models濡?紐⑤뱺 紐⑤뜽??吏묒쨷?섏뼱 ?덈떎怨?媛??(User confirmed this path)
